...
export const createMLP = (
  ...
  let layer = tf.layers
    .dense({ units: hidden_sizes[0], activation })
    .apply(input);
...
export abstract class Actor<Observation extends tf.Tensor> {
  ...
  abstract _log_prob_from_distribution(
    pi: Distribution,
    act: tf.Tensor
  ): tf.Tensor;
  abstract apply(
