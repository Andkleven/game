import { Environment, RenderModes } from "../index";
import { NotImplementedError } from "../../Errors";
import { PrimitiveBox2D } from "../../Spaces/primitiveBox2d";
import { Dict } from "../../Spaces/dict";
 ...
 */
export class SimpleGridWorld extends Environment<
  ObservationSpace,
  ActionSpace,
  Observation,
  State,
...
> {
  ...
  step(action: Action) {
    ...
    if (
      this.stepsSoFar >= this.maxEpisodeSteps &&
      this.maxEpisodeSteps !== -1
    ) {
      done = true;
    }
    this.stepsSoFar += 1;
  ...
  posIsInTargetPositions(pos: Position) {
    return this.targetPositions.some(
  ...
  public stateToRep(state: State): number {
    return (
      state.agentPos.x +
      state.agentPos.y * Math.max(state.grid.length, state.grid[0].length)
    );
  }
  ...
  dynamics(sucessorState: State, reward: Reward, state: State, action: Action) {
    ...
    if (!this.posOnGrid({ x, y })) {
      if (
        sucessorState.agentPos.x !== state.agentPos.x ||
        sucessorState.agentPos.y !== state.agentPos.y
      ) {
        return 0;
      }
    }
