...
export const tensorLikeToTensor = (x: TensorLike): Tensor => {
  ...
  return tensor(x.selection.data as number[]).reshape(x.shape);
};
export const NdArrayToTensorLike = (
  x: NdArray,
  target: TensorLike
): TensorLike => {
  ...
  if (index instanceof Array) {
    ...
    if (index.length === src.shape.length) {
      ...
      if (reduceMult(val.shape) !== reduceMult(requiredShape)) {
        throw new Error(
          `Cannot broadcast input array from shape ${val.shape} to ${requiredShape}`
        );
      }
      ...
      for (let i = 0; i < index.length; i++) {
        ...
        if (d >= src.shape[i]) {
          throw new Error(
            `Index ${d} out of bounds in ${i}th dimension of source array with shape ${src.shape}`
          );
        }
      }
    ...
    if (!arrayEqual(src.shape, index.shape)) {
      throw new Error(
        `Boolean mask shape ${index.shape} mismatched with ${src.shape}`
      );
    }
