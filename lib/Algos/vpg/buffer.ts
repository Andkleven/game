...
export class VPGBuffer {
  ...
  public store(
    ...
    if (this.ptr >= this.maxSize)
      throw new Error("Experience Buffer has no room");
  ...
  public finishPath(lastVal = 0) {
    ...
    this.advBuf
      .slice(path_slice)
      .assign(core.discountCumSum(deltas, this.gamma * this.lam), false);

    // compute rewards-to-go
    this.retBuf
      .slice(path_slice)
      .assign(core.discountCumSum(rews, this.gamma).slice([0, -1]), false);
  ...
  public async get(): Promise<VPGBufferComputations> {
    ...
    // normalization trick
    const stats = await ct.statisticsScalar(
      advBuf,
      { max: true, min: true },
      true
    );
