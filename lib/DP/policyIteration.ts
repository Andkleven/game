...
> extends Agent<State, Action> {
  ...
  private evaluatePolicy() {
    ...
    for (let step = 1; ; step++) {
      let delta = 0;
      if (
        this.configs.evaluatorConfigs.steps &&
        step >= this.configs.evaluatorConfigs.steps
      ) {
        break;
      }
      for (const rep of this.configs.allStateReps) {
        ...
        p_sp_r_s_a = this.dynamics.bind(env)(
          stepOut.observation,
          reward,
          obs,
          action
    ...
    for (let step = 1; ; step++) {
      ...
      for (const rep of this.configs.allStateReps) {
        ...
        for (const action of this.configs.allPossibleActions) {
          ...
          const stepOut = env.step(action);
          const reward = stepOut.reward;
          const p_sp_r_s_a = this.dynamics.bind(env)(
            stepOut.observation,
            reward,
            obs,
            action
          ...
          const value =
            p_sp_r_s_a * (reward + this.configs.discountRate * v_sp);
          if (value > bestValue) {
            bestValue = value;
            greedyAction = action;
          }
