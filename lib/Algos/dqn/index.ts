...
> extends Agent<Observation, Action> {
  ...
  constructor(
    ...
    if (!this.env.actionSpace.meta.discrete)
      throw new Error("Action space is not discrete");
  ...
  public act(observation: Observation): Action {
    ...
    const pred = this.policyNet.predict(
      this.obsToTensor(observation),
      {}
    ) as tf.Tensor;
  ...
  public async train(
    ...
    for (let t = 0; episodeIteration < configs.totalEpisodes; t++) {
      ...
      if (done) {
        ...
        if (
          t >= configs.learningStarts &&
          episodeIteration % configs.targetUpdateFreq === 0
        ) {
  ...
  private getEpsilon(
    ...
    const eps_threshold =
      epsEnd + (epsStart - epsEnd) * Math.exp((-1 * timeStep) / epsDecay);
  ...
  public async optimizeModel(configs: {
    ...
    const loss = tf.tidy(() => {
      ...
      const lossFunc = () => {
        ...
        const stateActionValues = stateValues
          .mul(tf.oneHot(actionBatch, stateValues.shape[1]!))
          .sum(-1); // this zeros out state values associated to unselected actions
        const nnn = this.targetNet.apply(nextStateBatch) as tf.Tensor;
        const nextStateValues = nnn.max(1);
        const doneMask = tf
          .scalar(1)
          .sub(tf.tensor1d(_doneBatch).asType("float32")); // mask with 1s where there exists a next state and 0s where it is finished and no next state
        const expectedStateActionValues = nextStateValues
          .mul(doneMask)
          .mul(configs.gamma)
          .add(rewardBatch);
