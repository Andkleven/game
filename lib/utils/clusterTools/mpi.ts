...
export const fork = async (forkCount: number) => {
  const listeningPromises: Array<Promise<Worker>> = [];
  if (cluster.isMaster) {
    ...
    for (const worker of workers) {
      workerInitSignals.push(
        send(
          { type: MessageType.INIT, data: [worker.id, forkCount + 1] },
          worker
        )
      );
    }
...
export const receiveFromWorker = async (worker: Worker): Promise<Message> => {
